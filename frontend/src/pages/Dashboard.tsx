...
function workflowBadge(status: string) {
  ...
  switch (status) {
    case 'approved':
      return <span className={`${base} border-primary/25 bg-primarySoft text-primary`}><CheckCircle2 className="w-3 h-3" />{WORKFLOW_LABELS[status]}</span>;
    case 'rejected':
      return <span className={`${base} border-primary/40 bg-primary/10 text-primary`}><XCircle className="w-3 h-3" />{WORKFLOW_LABELS[status]}</span>;
    case 'submitted':
      return <span className={`${base} border-primary/20 bg-primarySoft text-primary`}><SendHorizontal className="w-3 h-3" />{WORKFLOW_LABELS[status]}</span>;
    case 'revision_requested':
      return <span className={`${base} border-primary/30 bg-primary/5 text-primary`}><RotateCcw className="w-3 h-3" />{WORKFLOW_LABELS[status]}</span>;
    default:
      return <span className={`${base} border-border bg-background text-textSecondary`}>{WORKFLOW_LABELS[status] ?? status}</span>;
  }
}
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {[...Array(4)].map((_, i) => (
            <Card key={i}><CardContent className="pt-5 pb-5 h-24 animate-pulse rounded-xl bg-background">{null}</CardContent></Card>
          ))}
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <KPIBlock
          ...
          icon={AlertTriangle}
          accent={pendingForMe.length + pendingBudgets.length > 0 ? 'bg-primary/15 text-primary' : 'bg-background text-textSecondary border border-border'}
        />
        <KPIBlock
      ...
      {(pendingForMe.length > 0 || pendingBudgets.length > 0) && (
        <div className="flex flex-col gap-2">
          {pendingForMe.length > 0 && (
            <div className="flex items-center gap-3 rounded-lg border border-primary/20 bg-primarySoft px-4 py-3 text-sm">
              <AlertTriangle className="h-4 w-4 shrink-0 text-primary" />
              <span className="text-primary">
                <span className="font-semibold">{pendingForMe.length}</span> рабочих планов ожидают вашего согласования
              </span>
              <Link to="/work-plans" className="ml-auto flex items-center gap-1 font-medium text-primary hover:underline whitespace-nowrap">
              ...
              <div className="divide-y divide-border">
                ...
                {pendingBudgets.slice(0, 3).map((b) => (
                  <div key={b.id} className="flex items-center justify-between py-2.5 gap-3">
                    ...
                    <span className="whitespace-nowrap rounded-full border border-primary/20 bg-primarySoft px-2 py-0.5 text-xs font-medium text-primary">
                      {formatRub(b.planned)}
                    </span>
                  </div>
                ))}
