...
router.get('/', authenticate, (req: AuthRequest, res) => {
  ...
  const list = getInstructions(postId, allowed);
  res.json(list);
});
...
router.get('/:id/steps', authenticate, (req, res) => {
  ...
  const steps = getInstructionSteps(instructionId);
  res.json(steps);
});

/** Create step for an instruction. Admin or Department Head. */
router.post('/:id/steps', authenticate, requireRole('Admin', 'Department Head'), (req, res) => {
  const instructionId = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const instruction = getInstructionById(instructionId);
  if (!instruction) return res.status(404).json({ error: 'Instruction not found' });
  const { title, text, link, deadline, status, orderIndex } = req.body;
  if (!title?.trim()) return res.status(400).json({ error: 'title required' });
  const step = createInstructionStep(instructionId, {
    title: title.trim(),
    text: text ?? null,
    link: link ?? null,
...
router.put('/:id/steps/:stepId', authenticate, requireRole('Admin', 'Department Head'), (req, res) => {
  ...
  if (Object.keys(updates).length > 0) updateInstructionStep(stepId, updates as any);
  const step = getInstructionStepById(stepId);
  if (!step) return res.status(404).json({ error: 'Step not found' });
  res.json(step);
});

/** Delete instruction step. Admin or Department Head. */
router.delete('/:id/steps/:stepId', authenticate, requireRole('Admin', 'Department Head'), (req, res) => {
  const stepId = typeof req.params.stepId === 'string' ? req.params.stepId : req.params.stepId?.[0] ?? '';
...
router.post('/', authenticate, requireRole('Admin', 'Department Head'), (req, res) => {
  ...
  createInstruction({ id, title, postId, ownerPostId, status: status || 'draft', version: 1 });
  const created = getInstructionById(id);
  res.status(201).json(created);
});

/** Update instruction. Admin or Department Head only. */
router.put('/:id', authenticate, requireRole('Admin', 'Department Head'), (req, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const instruction = getInstructionById(id);
  if (!instruction) return res.status(404).json({ error: 'Instruction not found' });
  const { title, status, version } = req.body;
  updateInstruction(id, { title, status, version });
  const updated = getInstructionById(id);
  res.json(updated);
});

/** Delete instruction (and its steps). Admin or Department Head only. */
router.delete('/:id', authenticate, requireRole('Admin', 'Department Head'), (req, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const instruction = getInstructionById(id);
  if (!instruction) return res.status(404).json({ error: 'Instruction not found' });
