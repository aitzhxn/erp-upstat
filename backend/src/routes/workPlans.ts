...
router.get('/', authenticate, (req: AuthRequest, res) => {
  ...
  if (forMyApproval && myPostIds.length > 0) {
    list = getWorkPlans({ workflowStatus: 'submitted', approverPostIds: myPostIds });
  } else {
    const allowed = getAllowListForUser(req.user);
    list = getWorkPlans({ postId, allowedPostIds: allowed, workflowStatus: workflowStatus as any || undefined });
  }
  res.json(list);
});
...
router.get('/:id', authenticate, (req: AuthRequest, res) => {
  ...
  const tasks = getWorkPlanTasks(id);
  res.json({ ...plan, tasks });
});
...
router.put('/:id', authenticate, (req: AuthRequest, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const existing = getWorkPlanById(id);
  ...
  if (Object.keys(updates).length > 0) updateWorkPlan(id, updates as any);
  const updated = getWorkPlanById(id);
  res.json(updated!);
});
...
router.post('/:id/submit', authenticate, (req: AuthRequest, res) => {
  ...
  if (approver) {
    const allowed = getAncestorPostIds(plan.postId);
...
router.post('/:id/approve', authenticate, (req: AuthRequest, res) => {
  ...
  const ancestors = getAncestorPostIds(plan.postId);
  ...
  approveWorkPlan(id, typeof comment === 'string' ? comment : undefined);
  const updated = getWorkPlanById(id);
  res.json(updated!);
});

/** Reject work plan. Approver only. Comment required. */
router.post('/:id/reject', authenticate, (req: AuthRequest, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const { comment } = req.body || {};
  const plan = getWorkPlanById(id);
  if (!plan) return res.status(404).json({ error: 'Work plan not found' });
  ...
  rejectWorkPlan(id, commentStr);
  const updated = getWorkPlanById(id);
  res.json(updated!);
});

/** Request revision (manager): plan goes back to author with comment. Body: { comment? }. */
router.post('/:id/request-revision', authenticate, (req: AuthRequest, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const { comment } = req.body || {};
  const plan = getWorkPlanById(id);
  if (!plan) return res.status(404).json({ error: 'Work plan not found' });
  ...
  requestRevisionWorkPlan(id, commentStr);
  const updated = getWorkPlanById(id);
  res.json(updated!);
});

/** Get tasks of a work plan. */
router.get('/:id/tasks', authenticate, (req: AuthRequest, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const plan = getWorkPlanById(id);
  if (!plan) return res.status(404).json({ error: 'Work plan not found' });
  const allowed = getAllowListForUser(req.user);
...
router.post('/:id/tasks', authenticate, (req: AuthRequest, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  ...
  const created = createWorkPlanTask({ workPlanId: id, title: title.trim(), dueDate: dueDate ?? null, orderIndex: orderIndex ?? 0 });
  res.status(201).json(created);
});

/** Update task. */
router.put('/:id/tasks/:taskId', authenticate, (req: AuthRequest, res) => {
  ...
  const { title, dueDate, orderIndex } = req.body;
  const plan = getWorkPlanById(planId);
  if (!plan) return res.status(404).json({ error: 'Work plan not found' });
  const myPostIds = req.user?.id ? getPostsForUser(req.user.id).map(p => p.id) : [];
  const isAuthor = plan.authorUserId === req.user?.id || myPostIds.includes(plan.postId);
  const canEdit = (plan.workflowStatus === 'draft' || plan.workflowStatus === 'rejected' || plan.workflowStatus === 'revision_requested') && (isAuthor || req.user?.role === 'Admin');
  if (!canEdit) return res.status(403).json({ error: 'Cannot edit tasks' });
  updateWorkPlanTask(taskId, { title, dueDate, orderIndex });
  res.json({ ok: true });
});

/** Delete task. */
router.delete('/:id/tasks/:taskId', authenticate, (req: AuthRequest, res) => {
  const planId = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  const taskId = typeof req.params.taskId === 'string' ? req.params.taskId : req.params.taskId?.[0] ?? '';
  const plan = getWorkPlanById(planId);
  if (!plan) return res.status(404).json({ error: 'Work plan not found' });
...
router.get('/notifications/count', authenticate, (req: AuthRequest, res) => {
  if (!req.user?.id) return res.json({ count: 0 });
  const count = getWorkPlanNotificationCount(req.user.id);
  res.json({ count });
});

/** Get notifications for current user. */
router.get('/notifications', authenticate, (req: AuthRequest, res) => {
  if (!req.user?.id) return res.json([]);
  const limit = req.query.limit ? parseInt(req.query.limit as string, 10) : undefined;
  const notifications = getWorkPlanNotifications(req.user.id, limit);
  res.json(notifications);
});

/** Mark notification as read. */
router.patch('/notifications/:id/read', authenticate, (req: AuthRequest, res) => {
  const id = typeof req.params.id === 'string' ? req.params.id : req.params.id?.[0] ?? '';
  if (!req.user?.id) return res.status(401).json({ error: 'Unauthorized' });
  markWorkPlanNotificationAsRead(id);
  res.json({ ok: true });
});

/** Mark all notifications as read. */
router.post('/notifications/read-all', authenticate, (req: AuthRequest, res) => {
  if (!req.user?.id) return res.status(401).json({ error: 'Unauthorized' });
  markAllWorkPlanNotificationsAsRead(req.user.id);
  res.json({ ok: true });
});
